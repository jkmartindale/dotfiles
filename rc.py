...
# Inspired by https://dzone.com/articles/exit-python-shell
class Command(object):
    """A custom command to add to the interactive interpreter.

    No constructor arguments are used.
    
    This command implementation replaces hooking into `sys.excepthook` and catching `NameErrors`. The old implementation combined with the auto-import code became quite spaghetti, and it only hid `NameErrors` from the user instead of getting rid of them. This meant that if the user executed bad code in `block`, they would see the `NameError` turned up by `block` in addition to the error from their actual code.
    """
...
def command(function: Callable) -> Command:
